...
The transaction is owned by the caller (`AiResolutionService`) and includes intent, decision, plan, record, outcome, runtime effect, and revision writes. A retry with the same execution identity reuses the committed mechanical outcome and never rerolls, reapplies effects, or advances another revision.
...
`DiceService` is server-only and receives an injectable RNG in tests. It accepts only the allowlisted dice expressions used by the rules slice. Advantage/disadvantage selection and critical handling are server decisions.
...
Narration failure is persisted as a retryable narration attempt while the mechanical outcome and StateRevision remain committed. Retrying narration uses the same mechanical run/outcome and does not call `DiceService` or the mutation coordinator again. Player routes continue to rely on the existing server-side entry projection; Owner-only debug data remains behind Owner authorization.
