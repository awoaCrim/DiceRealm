...
/** 构建含当前 022 与未来 023 迁移的临时目录，验证 022 会被精确 allowlist 应用而 023 不会。 */
function migrationsWithFuture022(): { dir: string; manifestPath: string } {
  const dir = tempDir('dnd-enroll-mig22-');
  const names = ['001', '002', '003', '004', '005', '006', '007', '008', '009', '010', '012', '013', '014', '015', '016', '017', '018', '019', '020', '021', '022']
    .map((version) => readdirSync(COMMITTED_MIGRATIONS_DIR).find((name) => name.startsWith(`${version}_`))!)
    .sort();
  for (const name of names) {
    cpSync(join(COMMITTED_MIGRATIONS_DIR, name), join(dir, name));
  }
  writeFileSync(join(dir, '023_future_phase.sql'), 'CREATE TABLE IF NOT EXISTS future_phase_table (id INTEGER PRIMARY KEY);', 'utf8');
  const allNames = [...names, '023_future_phase.sql'].sort();
  const manifest = {
    format: 1,
...
describe('EnrollmentCoordinator init (final secure-ready contract)', () => {
  it('creates a fresh DB with current approved migration set, enrolls ready, session security ready, and the normal server gate accepts it', async () => {
    ...
    try {
      ...
      try {
        ...
        const versions = raw.prepare('SELECT version FROM platform_migrations ORDER BY version').all().map((r) => (r as { version: string }).version);
        expect(versions).toEqual(['001', '002', '003', '004', '005', '006', '007', '008', '009', '010', '012', '013', '014', '015', '016', '017', '018', '019', '020', '021', '022']);
      } finally {
        raw.close();
      }
    } finally {
  ...
  it('init applies exactly the frozen current approved migration set and never silently applies a future 023 from the manifest', async () => {
    const dir = tempDir('dnd-init-frozen-');
    try {
      const { dir: migrationsDir, manifestPath } = migrationsWithFuture022();
      const databasePath = join(dir, 'fresh.sqlite');
      const keyPath = join(dir, '.dnd-ai-credential-key');
      ...
      expect(result.enrollmentState).toBe('ready');
      expect(result.sessionSecurityState).toBe('ready');
      // 023 不得被静默应用（即使 manifest/迁移目录已含 023）。
      const raw = new Database(databasePath, { readonly: true });
      try {
        const versions = raw.prepare('SELECT version FROM platform_migrations ORDER BY version').all().map((r) => (r as { version: string }).version);
        expect(versions).toEqual(['001', '002', '003', '004', '005', '006', '007', '008', '009', '010', '012', '013', '014', '015', '016', '017', '018', '019', '020', '021', '022']);
