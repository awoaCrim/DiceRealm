...
export const defaultAiConfig: AiConfig = {
  ...
  outputFormatRules: '输出格式：只返回严格 JSON，字段为 publicLog、privateUpdatesByPlayer、ruleResults、interactionRequests；不要使用 Markdown 代码块。',
...
const fixedDndOutputContract = [
  ...
  '可选字段 characterResourceChanges：数组，每项包含 characterId/path/before/after/reason/ruleRefs。只修改 hitPoints.current/max/temp、hitDice.remaining、spellSlots.*、ammo.*、consumables.*、currency.*、conditions。before 必须与当前值一致。',
  '可选字段 diceRequests：数组，AI 可请求系统执行真实骰点。每项包含 type(abilityCheck/attackRoll/savingThrow/skillCheck/damage/healing)、ability/skill、dc、die、count、modifier、advantage、reason。characterId 可选，无角色时使用 NPC 数据或纯骰。系统会在 publicLog 中追加真实骰点结果。'
].join('\n');
