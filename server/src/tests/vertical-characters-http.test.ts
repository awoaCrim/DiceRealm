  ...
  it('runs register → login → owner create → player join → player create/update/submit → owner approve → player sees approved', async () => {
    ...
    try {
      ...
      const createRes = await fetch(`${baseUrl}/api/campaigns`, {
        method: 'POST',
        headers: jsonHeaders(owner.cookieHeader),
      ...
      const createChar = await fetch(`${charsBase}`, {
        method: 'POST',
        headers: jsonHeaders(playerA.cookieHeader),
      ...
      const ownerList = await fetch(`${charsBase}`, { headers: { cookie: owner.cookieHeader } });
      ...
      const aOwnView = (await (await fetch(`${charsBase}`, { headers: { cookie: playerA.cookieHeader } })).json()) as {
      ...
      const bList = await fetch(`${charsBase}`, { headers: { cookie: playerB.cookieHeader } });
      ...
      const bDraft = await fetch(`${charsBase}`, {
        method: 'POST',
        headers: jsonHeaders(playerB.cookieHeader),
      ...
      const aListText = await (await fetch(`${charsBase}`, { headers: { cookie: playerA.cookieHeader } })).text();
